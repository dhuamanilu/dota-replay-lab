...
def render_lua_tree(student: Any, preprocessor: Any) -> str:
...
def distill_policy(dataset: Path, model_path: Path, metrics_path: Path, output: Path, seed: int) -> dict[str, Any]:
    import joblib
    import numpy as np
    import pandas as pd
    ...
    output.write_text(render_lua_tree(student, bundle["preprocessor"]), encoding="utf-8")
...
def main() -> int:
    args = parse_args()
    result = distill_policy(args.dataset, args.model, args.metrics, args.output, args.seed)
